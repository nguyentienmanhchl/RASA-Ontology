    ...
    public static void insert(String filename) {
        try {
            ...
            while (iterator.hasNext()) {
                ...
                insert(m, key, "hasBorn", pretty(jsonObject.get("hasBorn").toString()));
                insert(m, key, "hasBornAt", pretty(jsonObject.get("hasBornAt").toString()));
                insert(m, key, "hasDied", pretty(jsonObject.get("hasDied").toString()));
                //insert(m, key, "hasJob", pretty(jsonObject.get("hasJob").toString()));
                //insert(m,key, "hasPeriod", pretty(jsonObject.get("hasPeriod").toString()));
                //insert(m,key, "hasSuccessor", pretty(jsonObject.get("hasSuccessor").toString()));
...
//            InitJena.insert("dbpedia_data_2.json");
